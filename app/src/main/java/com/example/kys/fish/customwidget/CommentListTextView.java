package com.example.kys.fish.customwidget;
...
public class CommentListTextView extends android.support.v7.widget.AppCompatTextView {
    ...
    public void setData (List<Comment> mInfos) {
        this.mInfos = mInfos;
        /**
    ...
    private SpannableStringBuilder getCommentString () {
        ...
        for (int mI = 0; mI < mInfos.size (); mI++) {
            final Comment mInfo = mInfos.get (mI);
            /**
             * 拼接成“张三：今天天气真好”这种形式，就是一行显示的文本。
             * 或者为张三 回复 李四：今天天气真好
             */
            String mS = null;
            if (mInfo.getReceiversName () == null || mInfo.getReceiversName ().equals ("")) {
                mS = mInfo.getSendName () + "：" + mInfo.getCommentContent ();
            } else {
                mS = mInfo.getSendName () + mTalkStr + mInfo.getReceiversName () + "：" + mInfo.getCommentContent ();
            }
            /**
             ...
             */
            int start = 0;
            int end = (mInfo.getSendName ()).length ();
            ...
            mString.setSpan (new ForegroundColorSpan(mTalkColor),mInfo.getSendName ().length (),mInfo.getSendName ().length ()+mTalkStr.length (), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);

            /**
             * 处理第二个人名
             */
            if (mInfo.getReceiversName () != null && !mInfo.getReceiversName ().equals ("")) {
                start = mInfo.getSendName ().length () + mTalkStr.length ();
                end = mInfo.getSendName ().length () + mTalkStr.length () + mInfo.getReceiversName ().length ();
                mString.setSpan (new ClickableSpan() {
                    @Override
                    public void updateDrawState (TextPaint ds) {
                        /**
    ...
    public interface onCommentListener {
         ...
         * @param mInfo    评论相关信息
         */
        public void onNickNameClick(int position, Comment mInfo);
         ...
         * @param mInfo    评论相关信息
         */
        public void onToNickNameClick(int position, Comment mInfo);
         ...
         * @param mInfo
         */
        public void onCommentItemClick(int position, Comment mInfo);
